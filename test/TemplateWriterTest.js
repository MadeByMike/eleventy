...
test("custom template engine", async (t) => {
  let tr = new TemplateRender("new", undefined);
  tr.extensionMap = new EleventyExtensionMap();

  let tw = new TemplateWriter(
    "./test/stubs/customEngine",
    "./test/stubs/customEngine/_customEngineSite"
  );
  let evf = new EleventyFiles(
    "./test/stubs/customEngine",
    "./test/stubs/customEngine/_customEngineSite",
  ...
  tr.config.extensionMap.add({
    ...
    key: "new",
    compile: () => () =>
      "Test content - this should not be included in the filename",
  });
  ...
  let tmpl = tw._createTemplate(files[0]);
  t.is(tmpl.inputDir, "./test/stubs/customEngine");
  t.is(
    await tmpl.getOutputPath(),
    "./test/stubs/customEngine/_customEngineSite/with-perma.html"
  );

  tmpl = tw._createTemplate(files[1]);
  t.is(tmpl.inputDir, "./test/stubs/customEngine");
  t.is(
    await tmpl.getOutputPath(),
    "./test/stubs/customEngine/_customEngineSite/without-perma/index.html"
  );
});
